...
def draw_power_overlay_screen(screen: pygame.Surface, font: pygame.font.Font, state: GameState) -> None:
    ...
    overlay = pygame.Surface((bar_width + 70, bar_height + 24), pygame.SRCALPHA)
    overlay.fill((0, 0, 0, 90))

    bar_rect = pygame.Rect(16, 12, bar_width, bar_height)
    ...
    if state.bort_speeds:
        ...
        for idx, speed in enumerate(state.bort_speeds, start=1):
            ...
            overlay.blit(label, (bar_rect.right + 14, pos_y - 8))
    ...
    for i in range(1, 10):
        ...
        if i % 5 == 0:  # подпись каждые 0.5
            ...
            overlay.blit(label, (bar_rect.right + 14, pos_y - label.get_height() // 2))

    # Подписи "4 борта" (верх) и "0" (низ)
    top_label = font.render("4 борта", True, TEXT_COLOR)
    bottom_label = font.render("0", True, TEXT_COLOR)
    overlay.blit(top_label, (bar_rect.right + 14, bar_rect.top - 6 - top_label.get_height() // 2))
    overlay.blit(bottom_label, (bar_rect.right + 14, bar_rect.bottom - bottom_label.get_height() // 2))
